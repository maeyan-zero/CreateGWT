...
public class CoordTransformImpl extends JavaScriptObject {
	...
	*/
	public static final native PointImpl localToGlobal(float x, float y, 
		...
		return $wnd.createjs.CoordTransform.localToGlobal(x, y, source);
	}-*/;
	...
	*/
	public static final native PointImpl globalToLocal(float x, float y, 
			DisplayObjectImpl target) /*-{
		return $wnd.createjs.CoordTransform.globalToLocal(x, y, source);
	}-*/;
	...
	*/
	public static final native PointImpl localToLocal(float x, float y, 
		...
		return $wnd.createjs.CoordTransform.localToLocal(x, y, source, target);
	}-*/;
	...
	*/
	public static final native Matrix2DImpl getConcatenatedMatrix(DisplayObjectImpl target) /*-{
		return $wnd.createjs.CoordTransform.getConcatenatedMatrix(target);
	}-*/;
	
	
	public static final native String getImageMask(ImageElement image, ImageElement mask, int width, int height) /*-{
		return $wnd.createjs.applyCanvasMask(image, mask, width, height, true);
	}-*/;
